odoo.define('partner_autocomplete.tests', function (require) {
            ...
            _.each(fieldsToPatch, function (fieldToPatch) {
                testUtils.mock.patch(fieldToPatch, {
                    _getBase64Image: function (url) {
                        return Promise.resolve(url === "rayl.app/logo.png" ? "odoobase64" : "");
                    },
                    _isOnline: function () {
                        return true;
                    },
    ...
    QUnit.test("Partner autocomplete : Company type = Company / Name search", async function (assert) {
        ...
        var form = await createView({
            ...
            assert.strictEqual($input.val(), "Odoo", "Input value should have been updated to \"RAYL\"");
            assert.strictEqual(form.$("input.o_field_widget").val(), "rayl.app", "website value should have been updated to \"rayl.app\"");
            ...
            await testUtils.nextTick();
            $dropdown = form.$(".o_field_partner_autocomplete .dropdown-menu:visible");
            assert.strictEqual($dropdown.length, 1, "there should be an opened dropdown when typing RAYL letters again");

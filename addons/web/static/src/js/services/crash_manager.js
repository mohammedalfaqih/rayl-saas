...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("RAYL Warning"),
            buttons: [
...
function session_expired(cm) {
    return {
        display: function () {
            const notif = {
                type: _t("RAYL Session Expired"),
                message: _t("Your RAYL session expired. The current page is about to be refreshed."),
            };
            const options = {
                buttons: [{
                    text: _t("Ok"),
